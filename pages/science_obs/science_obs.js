...
const PLAN_OPTIONS = [
  '丰富幼儿相关经验（如阅读科学绘本、观看视频）',
  '调整环境、材料（如增加种类、降低难度、设置问题情境）',
...
Page({
  data: {
    ...
    shareSelected: [false, false, false],
    shareRecord: '',
  ...
  loadObsItemsByLevel(levelIndex) {
    const obsItems = getObsItems('science', levelIndex);
  ...
  async loadData(id) {
    try {
      ...
      this.setData({
        ...
        shareSelected: obsData.shareSelected || [false, false, false],
